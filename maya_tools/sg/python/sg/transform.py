...
def freezeJoint( joints, evt=0 ):
    ...
    for joint in joints:
        mat = sg.convert.listToMatrix( cmds.getAttr( joint + '.m' ) )
...
def setOrientByChild( targetJnt, evt=0 ):
    ...
    sg.transform.lookAt( targetJnt, children )
...
def lookAt( rotTarget, aimTargets, evt=0 ):
    ...
    aimPos    = sg.selection.getCenter( aimTargets )
    direction = OpenMaya.MVector( aimPos - rotPos ).normal()
    
    maxDotValue, maxDotIndex = sg.value.maxDotIndexAndValue( direction, rotMtx )
...
def makeCenter( sels, evt=0 ):
    ...
    for sel in sels:
        ...
        mat = sg.convert.listToMatrix(matList)
        ...
        newMat = sg.convert.listToMatrix( newMatList )
...
def duplicateMirror( sels, evt=0 ):
    ...
    for sel in sels:
        ...
        duSel = cmds.rename( duSel[0], sg.convert.sideString( sel ) )
        duSels.append( duSel )
...
def createOrigPosition( dagNodes ):
    ...
    dagNodes = sg.convert.singleToList( dagNodes )
    
    for dagNode in dagNodes:
        sg.attribute.addAttr( dagNode, ln='origMatrix', dt='matrix')
...
def setToDefault( dagNodes ):
    ...
    for dagNode in sg.convert.singleToList( dagNodes ):
        mtx = sg.convert.matrixToList( OpenMaya.MMatrix() )
