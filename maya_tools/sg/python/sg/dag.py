...
def getLocalName( dagNode ):
    ...
    dagNode = OpenMaya.MFnDagNode( sg.base.getDagPath( dagNode ) )
    return dagNode.name()
...
def getDagNode( dagNodeName ):
    ...
    return OpenMaya.MFnDagNode( sg.base.getDagPath( dagNodeName ) )
...
def addIOShape( target ):
    ...
    oShape = sg.base.getMObject( targetShape )
...
def putObject( putTargets, typ='joint', evt=0 ):
    
    putTargets = sg.convert.singleToList( putTargets )
    center = sg.selection.getCenter( putTargets )
...
def putChild( putTarget, typ='transform', evt=0 ):
    ...
    sg.transform.setToDefault( putObject )
...
def getChildrenJoints( topNodes ):
    ...
    topNodes = sg.convert.singleToList( topNodes )
...
def makeParentAndReplaceConnect( targets, evt=0 ):
    ...
    for target in targets:
        ...
        sg.connect.getSourceConnection( targetP, target )
...
def makeCloneObject( target, cloneLabel= '_clone', **options  ):
    ...
    op_cloneAttrName = sg.value.getValueFromDict( options, 'cloneAttrName' )
    ...
    for cuTarget in targets:
        sg.attribute.addAttr( cuTarget, ln=attrName, at='message' )
...
def parentByOlder( targets ):
    ...
    for i in range( len( targets ) ):
        dagNode = OpenMaya.MFnDagNode( sg.base.getDagPath( targets[i] ) )
...
def makeChild( targetNode, name=None ):
    ...
    sg.transform.setToDefault( trNode )
