...
def setStdToMixamo( stdGrp, mixamoHipJnt ):
    ...
    for mixamoJnt, stdHandle, rotation in targets:
        rotMatrix    = sg.convert.rotateToMatrix( rotation )
        mixamoJntMtx = sg.convert.listToMatrix( cmds.getAttr( mixamoNs + mixamoJnt + '.wm' ) )
        mtx = sg.convert.matrixToList( rotMatrix * mixamoJntMtx )
...
def removeAnimationKey( mixamoGrp ):
    ...
    topJoint = sg.dag.getTopJointChildren( mixamoGrp )
...
def mixamoMeshToAutoRig( mixamoMeshGrp, autoRigGrp ):
    ...
    for mesh in meshs:
        ...
        allJoints += sg.dag.getNodeFromHistory( mesh, 'joint' )
    ...
    def getRigNs( rigGrp ):
        ...
        rigNs = ''
        for tr in trs:
            shape = sg.dag.getShape( tr )
            if not shape: continue
            if sg.dag.getLocalName(tr)[-7:] != 'Ctl_All': continue
            rigNs = sg.dag.getLocalName(tr).replace( 'Ctl_All', '' )
            break
        return rigNs
    ...
    def mixamoFacialRig( mixamoMeshs, duMeshs ):
        ...
        import MayaAutoControlRig.DJB_Character.FacialControls
        import MayaAutoControlRig.DJB_Character.CharacterNode
        ...
        for i in range( len( mixamoMeshs ) ):
            ...
            blendNodes = sg.dag.getNodeFromHistory( mixamoMesh, 'blendShape' )
            if not blendNodes: continue
            duMesh = duMeshs[i]
            srcMeshs = cmds.listConnections( blendNodes[0], s=1, d=0, type='mesh', shapes=1 )
            srcMeshs.append( sg.dag.getShape( duMesh ) )
            ...
            inst = MayaAutoControlRig.DJB_Character.FacialControls.FacialControls( srcMeshs )
            leftEye = MayaAutoControlRig.DJB_Character.CharacterNode.DJB_CharacterNode("LeftEye", optional_ = 1, parent = 'Ctl_Head', joint_namespace_ = '')
            RightEye = MayaAutoControlRig.DJB_Character.CharacterNode.DJB_CharacterNode("RightEye", optional_ = 1, parent = 'Ctl_Head', joint_namespace_ = '')
...
def createMixamoHandleForAutoRig( mixamoHip ):
    ...
    sg.attribute.addAttr( mixamoHip, ln='mocType', min=0, max=1, dv=1, cb=1 )
    ...
    def createHandle( handleName, rotation1, rotation2, axisIndex ):
        ...
        sg.transform.setToDefault( handle )
        ...
        dcmp = sg.connect.getDcmp( wtMtx )
        handle, handleP = sg.dag.makeParent( handle )[0]
