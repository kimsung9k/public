...
def connectMiddles( top, end, middles, vector ):
    ...
    lookAtTarget = sg.dag.putChild( top )
    lookAtTarget = sg.name.addName( lookAtTarget, top, 'lookAt' )
    
    sg.connect.lookAtConnect( end, lookAtTarget, vector )
    ...
    for i in range( len( middles ) ):
        ...
        child = sg.name.addName( child, lookAtTarget, 'child%02d' % i )
        
        child = cmds.parent( child, lookAtTarget )[0]
        dcmp= sg.connect.createLocalDcmp( end, lookAtTarget )
        ...
        sg.transform.setToDefault( child )
...
def putStdJnt( stdObject ):
    ...
    jnt = sg.name.replaceNameToTarget( stdObject, jnt, 'Std_', 'StdJnt_' )
...
def putStdJntHelp( stdObject ):
    ...
    stdJntHelp = sg.name.replaceNameToTarget( stdObject, stdJntHelp, 'Std_', 'StdJntHelp_' )
    
    sg.connect.constraint_parent( stdObject, stdJntHelp )
...
def makeLookAtChild( aimTarget, rotTarget ):
    rotTargetChild = sg.dag.putChild( rotTarget )
    sg.connect.lookAtConnect( aimTarget, rotTargetChild )
...
def makeFingerStdJnt( stds ):
    ...
    stdHelps.reverse()
    sg.dag.parentByOlder( stdHelps )
    stdHelps.reverse()
    
    stdJnts.reverse()
    sg.dag.parentByOlder( stdJnts )
    stdJnts.reverse()
    
    for i in range( len( stdHelps )-1 ):
        lookAtChild = makeLookAtChild( stdHelps[i+1], stdHelps[i] )
        sg.connect.constraint_parent( lookAtChild, stdJnts[i] )
    
    sg.connect.constraint_point( stdHelps[-1], stdJnts[-1] )
...
def makeIkLookBase( stdBase, stdMiddle, stdEnd, stdPoleV, minusX=False, minusY=False ):
    ...
    dcmpEnd = sg.connect.getLocalDcmp( stdEnd, stdBase )
    dcmpPoleV = sg.connect.getLocalDcmp( stdPoleV, stdBase )
    dcmpBase = sg.connect.getWorldDcmp( stdBase )
    ...
    stdLookBase = sg.dag.putChild( stdBase )
    stdLookBase = sg.name.replaceNameToTarget( stdBase, stdLookBase, 'Help_', 'HelpLookBase_' )
...
def makeIkMiddle( stdMiddle, ikLookBase ):
    ...
    dcmpMiddle = sg.connect.getLocalDcmp( stdMiddle, ikLookBase )
    ikMiddle = sg.dag.putChild( ikLookBase )
    ikMiddle = sg.name.replaceNameToTarget( stdMiddle, ikMiddle, 'Help_', 'HelpIkMiddle_' )
...
def makeIkOffset( stdMiddle, ikMiddle ):
    ...
    dcmp = sg.connect.getLocalDcmp( stdMiddle, ikMiddle )
    ikOffset = sg.dag.putChild( ikMiddle, 'joint' )
    ikOffset = sg.name.replaceNameToTarget( ikMiddle, ikOffset,'IkMiddle_', 'IkOffset_' )
...
def makeIkStdJnts( stds, minusX = False, minusY= False ):
    ...
    sg.connect.constraint_parent( baseLookAt, stdJntBase )
    sg.connect.constraint_parent( middleLookAt, stdJntMiddle )
    sg.connect.constraint_parent( stdEnd, stdJntEnd )

    sg.dag.parentByOlder( [stdJntEnd, stdJntMiddle, stdJntBase ] )

    sg.transform.setJointOrientZero([stdJntEnd, stdJntMiddle, stdJntBase ])
    sg.dag.parentByOlder([stdEnd, stdMiddle, stdBase ])
...
def makeFootStdJnts( stds ):
    ...
    sg.connect.constraint_parent( lookAtBase, stdJntBase )
    sg.connect.constraint_parent( lookAtMiddle, stdJntMiddle )
    sg.connect.constraint_point( stdEnd, stdJntEnd )

    stdJntBase = sg.name.replaceNameToTarget(stds[0], stdJntBase, 'Std_', 'StdFootJnt_' )
...
class MakeRigBase:
    ...
    def fixConnectBaseGrp(self, connector ):
        ...
        cmds.delete( cons )
        sg.connect.constraint_parent( connector, self.baseGrp )
    
    
    def makeParentGroup(self, stdJnt, addName ):
        
        baseGrp = cmds.createNode( 'transform', n='Grp_base_%s' % addName )
        sg.connect.constraint_parent( stdJnt, baseGrp )
        return baseGrp
...
class MakeBodyRig(MakeRigBase):
    ...
    def createRootCtl(self):
        ...
        sg.connect.translate( self.stdRoot, pCtlRoot )
        sg.connect.rotate( self.stdRoot, pCtlRoot )
        return ctlRoot
    ...
    def createPervisRotater(self):
        ...
        fnCtlPervis = OpenMaya.MFnDagNode( sg.base.getDagPath( ctlPervis ) )
        pCtlPervis = cmds.parent( pCtlPervis, self.ctlRoot )
        sg.transform.setToDefault( pCtlPervis )
        
        pervisWaistPointer = cmds.createNode( 'transform', n='pointer_to_waist' )
        sg.connect.translate( self.stdBack01, pervisWaistPointer )
        sg.connect.rotate( self.stdBack01, pervisWaistPointer )
    ...
    def createBodyRotator(self):
        ...
        sg.connect.constraint_point( self.pervPointer, pCtlRotator )
        ...
        fnCtlRotator = OpenMaya.MFnDagNode( sg.base.getDagPath( ctlRotator ) )
        pCtlRotator = cmds.parent( pCtlRotator, self.ctlRoot )
        sg.transform.setToDefault( pCtlRotator )
        ctlRotator = fnCtlRotator.partialPathName()
        
        fnCtlRotator2 = OpenMaya.MFnDagNode( sg.base.getDagPath( ctlRotator2 ) )
        pCtlRotator2 = cmds.parent( pCtlRotator2, ctlRotator )
        sg.transform.setToDefault( pCtlRotator2 )
    ...
    def createWaistCtl(self):
        ...
        ctlWaist, waistOffset = sg.dag.makeParent( ctlWaist )[0]
        
        fnCtlWaist = OpenMaya.MFnDagNode( sg.base.getDagPath( ctlWaist ) )
        ...
        sg.transform.setToDefault( pCtlWaist )
        ctlWaist = fnCtlWaist.partialPathName()
        
        halfMtx = sg.connect.getLocalHalfMatrixNode( self.ctlBodyRot1 )
        halfMtxDcmp = sg.connect.getDcmp( halfMtx )
        cmds.connectAttr( halfMtxDcmp + '.or', pCtlWaist + '.r' )
        sg.attribute.addAttr( self.ctlBodyRot1, ln='blend', min=0, max=1, dv=0.3, k=1 )
        cmds.connectAttr( self.ctlBodyRot1 + '.blend', halfMtx + '.blend' )
        
        pervPointerDcmp = sg.connect.getLocalDcmp( self.pervPointer, self.ctlRoot )
        cmds.connectAttr( pervPointerDcmp + '.ot', pCtlWaist + '.t' )
        
        dcmp = sg.connect.getLocalDcmp( self.ctlPervis, waistOffset )
        pointerHip = sg.dag.makeChild( ctlWaist, 'hipPointer' )
        cmds.connectAttr( dcmp + '.ot', pointerHip + '.t' )
        cmds.connectAttr( dcmp + '.or', pointerHip + '.r' )
        
        pointerChestLower = sg.dag.makeChild( waistOffset, 'pointer_chestLower' )
        pointerChest = sg.dag.makeChild( pointerChestLower, 'pointer_chest' )
        
        sg.connect.constraint_point( self.ctlBodyRot2, pointerChestLower )
        sg.connect.constraint_point( sg.dag.getParent( self.ctlChest ), pointerChest )
    ...
    def createHipCtl(self):
        ...
        fnCtlHip = OpenMaya.MFnDagNode( sg.base.getDagPath( ctlHip ) )
        pCtlHip = cmds.parent( pCtlHip, self.ctlRoot )[0]
        ctlHip = fnCtlHip.partialPathName()
        sg.connect.constraint_parent( self.pointerHip, pCtlHip )
    ...
    def createBindedCurve(self):
        ...
        bindedGrp = sg.dag.makeChild( self.ctlRoot, 'bindedTransformGrp' )
        
        bindPre01 = sg.dag.getParent( self.ctlHip )
        bindPre02 = sg.dag.getParent( self.ctlChest )
        
        bind01 = sg.dag.makeBrother( self.ctlHip, 'back_bind01' )
        bind02 = sg.dag.makeBrother( self.ctlChest, 'back_bind02' )
        ...
        pointsObjs = [ self.pointerHip, sg.dag.getParent(self.ctlWaist), sg.dag.getParent(self.pointerChest), self.pointerChest ]
    ...
    def createSplineJoints(self):
        ...
        topJoint, ikHandle = sg.ik.createIkSplineHandleJoints( self.curve, self.ctlRoot )
        ...
        sg.connect.constraint_parent( self.ctlHip, jnt )
        topJoint = cmds.parent( topJoint, jnt )[0]
        
        children = cmds.listRelatives( topJoint, c=1, ad=1, type='joint' )
        sg.connect.constraint_orient( self.ctlChest, children[0] )
        ...
        for i in range( len( splineJoints ) ):
            fnSplineJoints.append( OpenMaya.MFnDagNode( sg.base.getDagPath(splineJoints[i]) ) )
    ...
    def addHipConnector(self, stdHipJnt_L_, stdHipJnt_R_ ):
        ...
        rootChild_L_ = sg.dag.makeChild( self.ctlRoot, 'pointer_hip_L_inRoot' )
        rootChild_R_ = sg.dag.makeChild( self.ctlRoot, 'pointer_hip_R_inRoot' )
        hipChild_L_  = sg.dag.makeChild( self.ctlHip, 'pointer_hip_L_inHip' )
        hipChild_R_  = sg.dag.makeChild( self.ctlHip, 'pointer_hip_R_inHip' )
        ...
        connector_L_ = sg.dag.makeChild( self.topRigJoint, 'connector_hip_L_' )
        connector_R_ = sg.dag.makeChild( self.topRigJoint, 'connector_hip_R_' )
        sg.attribute.addAttr( connector_L_, ln='connector', at='message' )
        sg.attribute.addAttr( connector_R_, ln='connector', at='message' )
        
        sg.connect.constraint_orient( rootChild_L_, connector_L_ )
        sg.connect.constraint_orient( rootChild_R_, connector_R_ )
        sg.connect.constraint_point( hipChild_L_, connector_L_ )
        sg.connect.constraint_point( hipChild_R_, connector_R_ )
        
        self.hipConnector_L_ = connector_L_
        self.hipConnector_R_ = connector_R_
        
        hipConnectRotator = sg.dag.makeChild( self.ctlRoot, 'rotator_hipConnector' )
        localDcmp = sg.connect.getLocalDcmp( self.ctlHip, self.ctlRoot )
        ...
        dcmpHipConnector = sg.connect.getDcmp( mmForHip )
    ...
    def _getChestInverseMatrix(self):
        ...
        dcmpTrans = sg.connect.getDcmp( self.stdChest )
        dcmpRotate = sg.connect.getDcmp( self.stdBack02 )
    ...
    def addCollarConnector(self, stdCollarJnt_L_, stdCollarJnt_R_ ):
        ...
        child_L_ = sg.dag.makeChild( endJnt, 'connector_collar_L_' )
        child_R_ = sg.dag.makeChild( endJnt, 'connector_collar_R_' )
        sg.attribute.addAttr( child_L_, ln='connector', at='message' )
        sg.attribute.addAttr( child_R_, ln='connector', at='message' )
        ...
        dcmp_L_ = sg.connect.getDcmp( mm_L_ )
        dcmp_R_ = sg.connect.getDcmp( mm_R_ )
    ...
    def addNeckConnector(self, stdJntNeck ):
        ...
        endJnt = cmds.listRelatives( self.topRigJoint, c=1, ad=1, type='joint', f=1 )[0]
        connector = sg.dag.makeChild( endJnt, 'connector_neck' )
        sg.attribute.addAttr( connector, ln='connector', at='message' )
...
class MakeArmRig(MakeRigBase):
    ...
    def __init__(self, *stdJnts, **options ):
        ...
        self.name = sg.value.getValueFromDict( options, 'name' )
        self.side = sg.value.getValueFromDict( options, 'side' )
        self.sep  = sg.value.getValueFromDict( options, 'sep' )
    ...
    def makeFKs( self ):
        ...
        ctlFirst  = sg.name.replaceNameToTarget( self.stdStart, ctlFirst, 'StdJnt_', 'Ctl_FK_' )
        ctlSecond = sg.name.replaceNameToTarget( self.stdMiddle, ctlSecond, 'StdJnt_', 'Ctl_FK_' )
        ctlThird  = sg.name.replaceNameToTarget( self.stdEnd, ctlThird, 'StdJnt_', 'Ctl_FK_' )
        
        sg.dag.makeParent( [ctlFirst,ctlSecond,ctlThird])
        ...
        rigJntFirst  = sg.name.replaceNameToTarget( self.stdStart,  rigJntFirst,  'StdJnt_', 'JntFk_' )
        rigJntSecond = sg.name.replaceNameToTarget( self.stdMiddle, rigJntSecond, 'StdJnt_', 'JntFk_' )
        rigJntThird  = sg.name.replaceNameToTarget( self.stdEnd,  rigJntThird,  'StdJnt_', 'JntFk_' )
    
        sg.connect.constraint_parent( ctlFirst,  rigJntFirst )
        sg.connect.constraint_parent( ctlSecond, rigJntSecond )
        sg.connect.constraint_parent( ctlThird,  rigJntThird )
        ...
        sg.transform.setToDefault( ctlFirstP )
        sg.transform.setJointOrientZero( rigJntFirst )
    ...
    def makeIKs( self ):
        ...
        dcmpThird = sg.connect.getLocalDcmp( self.stdEnd, self.stdStart )
        
        ctlFirst  = cmds.circle( ch=1, o=1, nr = [1,0,0] )[0]
        ctlFirst = cmds.rename( ctlFirst, 'Ctl_%sIK_%s_' %( self.name, self.side ) )
        sg.dag.makeParent( ctlFirst )
        ctlFirstP = cmds.listRelatives( ctlFirst, p=1, f=1 )[0]
        fnCtlFirst = sg.dag.getDagNode( ctlFirst )
        ...
        rigJntFirst = sg.name.replaceNameToTarget( self.stdStart, rigJntFirst, 'StdJnt_', 'JntIk_' )
        rigJntSecond = sg.name.replaceNameToTarget( self.stdMiddle, rigJntSecond, 'StdJnt_', 'JntIk_' )
        rigJntThird = sg.name.replaceNameToTarget( self.stdEnd, rigJntThird, 'StdJnt_', 'JntIk_' )
        
        rigJntFirst = cmds.parent( rigJntFirst, self.baseGrp )[0]
        sg.transform.setToDefault( rigJntFirst )
        ...
        sg.connect.constraint_point( ctlFirst, handle )
        ...
        sg.dag.makeParent( ctlPoleV )
        ctlPoleVP = cmds.listRelatives( ctlPoleV, p=1, f=1 )[0]
        fnCtlPoleV = sg.dag.getDagNode( ctlPoleV )
        
        dcmpPoleV = sg.connect.getLocalDcmp( self.stdPoleV, self.stdStart )
        ...
        dcmpPoleV = sg.connect.getLocalDcmp( ctlPoleV, self.baseGrp )
        cmds.connectAttr( dcmpPoleV + '.ot', handle + '.poleVector' )
        handle = cmds.parent( handle, self.baseGrp )[0]
        
        sg.connect.constraint_orient( ctlFirst, rigJntThird )
    ...
    def fixPoleVector(self):
        ...
        lookAtPoleV = sg.connect.makeLookAtChild( self.ikGrp, self.ctlIk, 'lookAt_PoleV_%s_' % self.side )
        ctlPoleVP = cmds.parent( ctlPoleVP, lookAtPoleV )[0]
        lookDcmp = sg.connect.getLocalDcmp( self.stdEnd, self.stdStart )
        angleNode = cmds.createNode( 'angleBetween' )
    ...
    def makeBlendedJoints(self):
        ...
        sg.attribute.addAttr( ctlSwitch, ln='fkSwitch', min=0, max=1, dv=0, k=1 )
        ...
        blendJntFirst = sg.name.replaceNameToTarget( self.stdStart, blendJntFirst, 'StdJnt_', 'Jnt_' )
        blendJntSecond = sg.name.replaceNameToTarget( self.stdMiddle, blendJntSecond, 'StdJnt_', 'Jnt_' )
        blendJntThird = sg.name.replaceNameToTarget( self.stdEnd, blendJntThird, 'StdJnt_', 'Jnt_' )
        sg.attribute.addAttr( blendJntThird, ln='connector', at='message' )
        
        blendJnts = [blendJntFirst, blendJntSecond, blendJntThird]
        
        for i in range( 3 ):
            blendTwoMatrixNode = sg.connect.getBlendTwoMatrixNode( self.iks[i], self.fks[i], None, True )
            dcmp = sg.connect.getDcmp( blendTwoMatrixNode )
        ...
        fnCtlSwitch = sg.dag.getDagNode( ctlSwitch )
        sg.connect.constraint_parent( blendJntThird, pCtlSwitch )
    ...
    def setVisConnections(self):
        ...
        sg.attribute.addAttr( self.ctlSwitch, ln='fkVis', dv=0, cb=1, min=0, max=1, at='long' )
        sg.attribute.addAttr( self.ctlSwitch, ln='ikVis', dv=1, cb=1, min=0, max=1, at='long' )
    ...
    def createCurve(self):
        ...
        dcmpPoint1 = sg.connect.getLocalDcmp( self.blendJntFirst, self.baseGrp )
        dcmpPoint2 = sg.connect.getLocalDcmp( self.blendJntOffset, self.baseGrp )
        dcmpPoint3 = sg.connect.getLocalDcmp( self.blendJntThird, self.baseGrp )
        ...
        sg.transform.setToDefault( curve1 )
        sg.transform.setToDefault( curve2 )
    ...
    def createSplineJoints(self, numSep=3 ):
        ...
        topJoint1, ikHandle1 = sg.ik.createIkSplineHandleJoints( self.curve1, self.baseGrp, numSep )
        topJoint2, ikHandle2 = sg.ik.createIkSplineHandleJoints( self.curve2, self.baseGrp, numSep )
        ...
        upObjectArm0 = sg.dag.makeChild( self.baseGrp, 'upObject%s0_%s_' % ( self.name, self.side ) )
        sg.connect.lookAtConnect( self.blendJntSecond, upObjectArm0 )
        
        pLookAtUpObjectArm1 = sg.dag.makeChild( self.blendJntSecond, 'PlookAtUpObject%s1_%s_' % ( self.name, self.side ) )
        lookAtUpObjectArm1  = sg.dag.makeChild( pLookAtUpObjectArm1, 'lookAtUpObject%s1_%s_' % ( self.name, self.side ) )
        upObjectArm1 = sg.dag.makeChild( pLookAtUpObjectArm1, 'upObject%s1_%s_' % ( self.name, self.side ) )
        cmds.connectAttr( self.blendJntThird + '.t', pLookAtUpObjectArm1+'.t' )
        aimTarget = sg.dag.makeChild( self.blendJntThird, 'aimTarget%s1_%s_' % ( self.name, self.side ) )
        cmds.setAttr( aimTarget + '.t', 1,0,0 )
        sg.connect.lookAtConnect( aimTarget, lookAtUpObjectArm1 )
        dcmp = sg.connect.getLocalDcmp( self.blendJntThird, lookAtUpObjectArm1 )
        ...
        fnJoints = []
        for joint in joints:
            fnJoint = OpenMaya.MFnDagNode( sg.dag.getDagPath( joint ) )
        ...
        sg.connect.constraint_orient( self.blendJntThird, fnJoints[-1].partialPathName() )
    ...
    def fixIKCtlOrientation(self, stdJntGrp, ctlWorldGrp ):
        ...
        pCtlIk       = sg.dag.getParent( self.ctlIk )
        pCtlIkTransMtx = sg.connect.getLocalMatrix( self.stdEnd, stdJntGrp )   
        ikGrpWorldCtlLocal = sg.connect.getLocalMatrix( self.ikGrp, ctlWorldGrp )
        dcmpTranslate = sg.connect.getLocalDcmp( pCtlIkTransMtx, ikGrpWorldCtlLocal )
        dcmpRotate    = sg.connect.getLocalDcmp( stdJntGrp, ikGrpWorldCtlLocal )
        ...
        ctlShape = sg.dag.getShape( beforeIkCtl )
        cmds.select( pCtlIk )
        newCtl = cmds.joint( radius= 0, n= self.ctlIk )
        cmds.parent( ctlShape, newCtl, add=1, shape=1 )
        
        wristLocalMtx = sg.connect.getLocalMatrix( self.stdEnd, stdJntGrp )
        wristDcmp = sg.connect.getDcmp( wristLocalMtx )
    ...
    def addFootJointRig(self, footBaseGrp, aimTarget1, aimTarget2, ikConnector, stdFootAnkle, stdFootToe, stdFootToeEnd ):
        ...
        sg.connect.constraint_parent( self.ctlIk, footBaseGrp )
        
        cons = cmds.listConnections( self.ikHandle + '.t', s=1, d=0, p=1, c=1 )
        cmds.disconnectAttr( cons[1], cons[0] )

        sg.connect.constraint_point( ikConnector, self.ikHandle )
        ...
        pToeIk = sg.dag.makeChild( footIk, 'PRigJntIk_toe_%s_' % self.side )
        ...
        pToeFk = sg.dag.makeChild( footFk, 'PJntFk_toe_%s_' % self.side )
        ...
        pToeBlend = sg.dag.makeChild( footBlend, 'PRigJnt_toe_%s_' % self.side )
        ...
        pCtlToe, ppCtlToe = sg.dag.makeParent( pCtlToe, footBaseGrp )[0]
        
        axisIndex = 0
        if self.side == 'R': axisIndex += 3
        sg.connect.lookAtConnect( aimTarget2, pCtlToe, axisIndex )
        sg.connect.lookAtConnect( aimTarget1, footIk, axisIndex )
        ...
        sg.connect.constraint_parent( pToeIk, ppCtlToe )
        sg.connect.constraint_parent( ctlToe, toeIk )
        ...
        stdAnkle = sg.dag.getParent( stdFootAnkle )
        dcmp = sg.connect.getLocalDcmp( stdFootToe, stdAnkle )
        ...
        for i in range( 2 ):
            blendTwoMatrixNode = sg.connect.getBlendTwoMatrixNode( footIks[i], footFks[i], None, True )
            dcmp = sg.connect.getDcmp( blendTwoMatrixNode )
    ...
    def followConnect(self, stdJntGrp, stdJnts, controllers ):
        ...
        for i in range( len( stdJnts ) ):
            ...
            followPtr = sg.dag.makeChild( controllers[i], followAttrName )
        ...
        followMatrix = sg.connect.getFollowMatrix( mms )
        dcmp = sg.connect.getDcmp( followMatrix )
        
        cmds.connectAttr( dcmp + '.ot', pCtl + '.t', f=1 )
        cmds.connectAttr( dcmp + '.or', pCtl + '.r', f=1 )
        
        for i in range( len( followNames ) ):
            sg.attribute.addAttr( self.ctlSwitch, ln='%s' % followNames[i], k=1, min=0, max=1, dv=0 )
...
class MakeCollarRig(MakeRigBase):
    ...
    def createJoint(self):
        ...
        dcmpCtl = sg.connect.getLocalDcmp( self.ctl, self.baseGrp )
        ...
        sg.attribute.addAttr( self.ctl, ln='stretch', k=1, min=0, max=1 )
        ...
        sg.connect.lookAtConnect( self.ctl, topJnt )
        
        endObject = sg.dag.makeChild( self.baseGrp, 'RigObject_collar_%s_end' % self.side )
        cmds.connectAttr( self.stdCollarEnd + '.t', endObject + '.t' )
        cmds.connectAttr( self.stdCollarEnd + '.r', endObject + '.r' )
        sg.connect.constraint_orient( endObject, endJnt )
...
class MakeHandRig(MakeRigBase):
    ...
    def getStdJnt( self, name ):
        ...
        for target in targets:
            if target.find( self.handStdJnt ) != -1:
                return sg.dag.getLocalName( target )
    ...
    def createCtls(self, stdJntTop ):
        ...
        for i in range( len( children )-1 ):
            ctl, pCtl = controller.getCircleController( [1,0,0], 0.3, sg.dag.getLocalName(children[i]).replace( 'StdJnt_', 'Ctl_' ) )
            fnCtl = sg.dag.getDagNode( ctl )
    ...
    def createJoints(self, stdJntTop, ctls ):
        ...
        for i in range( len( lookAtObjs ) ):
            ...
            sg.connect.constraint_parent( lookAtObjs[i], jnt )
            cmds.select( jnt )
            rigJnts.append( jnt )
        ...
        sg.connect.constraint_parent( ctls[-1], lastJnt )
...
class MakeFootRig(MakeRigBase):
    ...
    def createController(self ):
        ...
        fnCtl = sg.dag.getDagNode( ctl )
        pCtl = cmds.parent( pCtl, self.baseGrp )[0]
        sg.connect.connect( self.stdFootPivot, pCtl, ['t', 'r'], ['t', 'r'] )
    ...
    def createRigJoints(self):
        ...
        dcmpFootCenter = sg.connect.getLocalDcmp( self.stdFootCenter, self.footStdJnt )
        sg.connect.connect( dcmpFootCenter, self.rigJntFootCenter, ['ot', 'or'], ['t', 'jo'] )
        dcmpFootPivot = sg.connect.getLocalDcmp( self.stdFootPivot, self.stdFootCenter )
        sg.connect.connect( dcmpFootPivot, self.rigJntFootPivot, ['ot', 'or'], ['t', 'jo'] )
        sg.connect.connect( self.stdFootCenter, self.rigJntFootBall, ['t'], [ 't'] )
        sg.connect.connect( self.stdFootBankIn, self.rigJntFootBandIn, ['t'], ['t'] )
        sg.connect.connect( self.stdFootBankOut, self.rigJntFootBandOut, ['t'], ['t'] )
        sg.connect.connect( self.stdFootRotateEnd, self.rigJntFootRotateEnd, ['t'], ['t'] )
        dcmpFootToe = sg.connect.getLocalDcmp( self.stdFootToe, self.stdFootRotateEnd )
        sg.connect.connect( dcmpFootToe, self.rigJntFootToe, ['ot', 'or'], ['t', 'jo'] )
        dcmpFootEnd = sg.connect.getLocalDcmp( self.stdFootAnkle, self.stdFootToe )
        sg.connect.connect( dcmpFootEnd, self.rigJntFootEnd, ['ot', 'or'], ['t', 'jo'] )
    ...
    def createConnection(self):
        ...
        sg.attribute.addAttr( self.ctlFoot, ln='walkRoll', k=1 )
        sg.attribute.addAttr( self.ctlFoot, ln='footEndAngle', min=1, dv=10, max=90 )
        sg.attribute.addAttr( self.ctlFoot, ln='walkRollAngle', k=1, min=1, dv=30, max=90 )
        sg.attribute.addAttr( self.ctlFoot, ln='bank', k=1 )
        
        dcmp = sg.connect.getLocalDcmp( self.stdToeEnd, self.stdFootCenter )
        angleNode = cmds.createNode( 'angleBetween' )
...
class MakeHeadRig( MakeRigBase ):
    ...
    def createController(self):
        ...
        fnCtlNeck = OpenMaya.MFnDagNode( sg.base.getDagPath(ctlNeck) )
        fnCtlHead = OpenMaya.MFnDagNode( sg.base.getDagPath(ctlHead) )
        ...
        sg.connect.getSourceConnection( pCtlNeck, self.baseJnts[1] )
        cmds.setAttr( pCtlNeck + '.r', 0,0,0 )
        sg.connect.constraint_orient( ctlNeck, self.baseJnts[1] )
        dcmp = sg.connect.getBlendTwoMatrixDcmpNode( ctlNeck, pCtlNeck, self.baseJnts[0] )
        cmds.connectAttr( dcmp + '.or', self.baseJnts[0] + '.r' )
        sg.connect.constraint_point( self.baseJnts[2], pCtlHead )
    ...
    def createResults(self):
        ...
        headPointer = sg.dag.makeChild( pCtlHead, 'PointerCtl_head' )
        cmds.connectAttr( self.ctlHead + '.t', headPointer + '.t' )
        
        wtAddMtx = sg.connect.createSkinWeightMatrix( [self.baseGrp, pCtlHead], [self.baseGrp, headPointer] )
        ...
        neckCenter = sg.dag.makeChild( self.baseGrp, 'Pointer_neck' )
        ...
        rigJntResultGrp = sg.dag.makeChild( self.baseGrp, 'PRigJnt_Neck' )
        resultJnt0 = cmds.joint( n='RigJnt_Neck' )
        pResultJnt1 = sg.dag.makeChild( resultJnt0, 'PRigJnt_NeckMiddle' )
        ...
        sg.connect.lookAtConnect( neckCenter, resultJnt0 )
        sg.connect.constraint_point( neckCenter, pResultJnt1 )
        sg.connect.lookAtConnect( headPointer, resultJnt1 )
        sg.connect.constraint_parent( self.ctlHead, resultJnt2 )
        
        pResultJnt1 = sg.dag.getParent( resultJnt1 )
        dcmpPHead = sg.connect.getDcmp( pCtlHead )
        dcmpHead = sg.connect.getDcmp( self.ctlHead )
        ...
        dcmpForAngle = sg.connect.getDcmp( mm )
        ...
        dcmp = sg.connect.getDcmp( addMtx )
    ...
    def createEyeRig(self):
        ...
        rigJntEye_L_, pRigJntEye_L_ = sg.dag.makeParent( rigJntEye_L_ )[0]
        rigJntEye_R_, pRigJntEye_R_ = sg.dag.makeParent( rigJntEye_R_ )[0]
        ...
        sg.connect.lookAtConnect( ctlEye_L_, rigJntEye_L_ )
        sg.connect.lookAtConnect( ctlEye_R_, rigJntEye_R_ )

        sg.attribute.addAttr( ctlEye, ln='distance', min=0.01, dv=10, k=1 )
...
def makeSkinJoints( sep ):
    ...
    for jnt in rigJnts:
        ...
        if jntP.split( '_' )[0] in ['RigJnt']:
            ...
            sg.connect.constraint_parent( jnt, newJnt )    
        
        newJnts.append( sg.dag.getDagNode( newJnt ) )
    ...
    for i in range( len( rigJnts ) ):
        ...
        if cmds.objExists( newJntP ) and newJntP.find( 'SkinJnt_' ) != -1:
            ...
            sg.transform.setToDefault( newJnts[i].partialPathName() )
            cmds.setAttr( newJnt + '.jo', 0,0,0 )
    
    for side in ['L', 'R']:
        jnt = cmds.parent( 'SkinJnt_eye_%s_' % side, 'SkinJnt_head' )[0]
        sg.transform.setJointOrientZero( jnt )
        ...
        for finger in fingers:
            ...
            sg.transform.setJointOrientZero( finger )
...
def editControllerShape():
    ...
    f = open( os.path.dirname(sg.Function_autoRig.__file__) + '/ctlShapeInfo.txt', 'r' )
...
def makeControllerHistory():
    ...
    for ctl in ctls:
        ctlShape = sg.dag.getShape( ctl )
        ctlShape = cmds.rename( ctlShape, ctl + 'Shape' )
        
        ctlOrig = sg.dag.addIOShape( ctl )
...
class MakeRig:
    ...
    def __init__(self, stdGroup, sep=3 ):
        ...
        sg.connect.constraint_scale( ns + 'Ctl_All', ns + 'Grp_skinJnt' )
...
def getMocapJoint( grpSet ):
    ...
    mocJntTopName = sg.dag.getLocalName(stdJntTop).replace( 'StdJnt_', 'MocJnt_' )
    ...
    for mocJoint in mocJoints:
        mocJoint = cmds.rename( mocJoint, sg.dag.getLocalName( mocJoint ).replace( 'StdJnt_', 'MocJnt_' ) )
