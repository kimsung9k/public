...
class Weight_perVtx:
    ...
    def setWeight(self, jointIndices, weights ):
        ...
        sg.attribute.removeMultiInstances( self.plugWeights.name() )
...
def printMatrix( mtx ):
    ...
    return sg.convert.matrixToList( mtx )
...
class Node:
    ...
    def __init__(self, skinCluster ):
        ...
        self.fnNode = OpenMaya.MFnDependencyNode( sg.base.getMObject( skinCluster ) )
...
def replaceSkinClusterJoint( jntBefore, jntAfter, skinedMesh ):
    ...
    skinClusterNode = sg.get.nodeFromHistory( skinedMesh, 'skinCluster', pdo=1 )
...
def inverseSkinClusterConnect( origMesh, targetShape, skinedMesh ):
    ...
    origShape = sg.dag.getShape( origMesh )
    skinShape = sg.dag.getShape( skinedMesh )
    targetShape = sg.dag.getShape( targetShape )
    
    skinNode = sg.get.nodeFromHistory( skinShape, 'skinCluster', pdo=1 )[0]
    
    fixObj = cmds.duplicate( origShape )[0]
    fixShape = sg.dag.getShape( fixObj )
...
def combineSkinCluster( skinedObjs ):
    ...
    skinedObjs = sg.dag.getMeshTransforms( skinedObjs )
    
    for skinedObj in skinedObjs:
        objShape = sg.dag.getShape( skinedObj )
        fnMesh = OpenMaya.MFnMesh( sg.dag.getDagPath( objShape ) )
        if not numVertices:
    ...
    for skinedObj in skinedObjs:
        ...
        skinCluster = sg.dag.getNodeFromHistory( skinedObj, 'skinCluster' )
        if not skinCluster: continue
        
        skinCluster = skinCluster[0]
        
        fnNode = OpenMaya.MFnDependencyNode( sg.base.getMObject( skinCluster ) )
    ...
    srcShape = sg.dag.getShape( skinedObjs[0] )
    newMeshObj = sg.dag.getParent( newMesh )
...
def setBindPreDefault( meshs ):
    ...
    for sel in meshs:
        ...
        fnSkinNode = OpenMaya.MFnDependencyNode( sg.base.getMObject( skinNode ) )
